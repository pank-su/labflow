# labflow

**Универсальный workflow для лабораторных, практических и курсовых работ.**

`labflow` помогает пройти путь от методички до проверенного результата:
извлечь требования, реализовать программную или математическую часть, собрать
отчёт и провести self-review.

![labflow](banner.jpg)

Репозиторий не содержит правил конкретного вуза. Универсальное ядро можно
дополнять отдельными domain-specific skills, например
[`guap-skill`](https://github.com/pank-su/guap-skill).

## Установка

Для Claude Code, Codex CLI и OpenCode запусти из каталога проекта:

```bash
npx skills add pank-su/labflow --copy
```

Установщик покажет найденные skills и доступные harness’ы. Выбери нужные
компоненты, область установки и подтверди действие через `yes`.

Typst-адаптер устанавливается отдельно, когда он нужен: его можно выбрать в
интерактивном списке `npx skills`.

## Основные skills

- `labflow` — общий workflow от задания до результата;
- `labflow-context` — требования, ограничения и deliverables;
- `labflow-coding` — программная часть и тесты;
- `labflow-math` — воспроизводимые вычисления, формулы и графики;
- `labflow-report` — отчёт из проверенных артефактов;
- `labflow-self-review` — проверка требований, кода, математики и оформления;
- `labflow-typst` — отдельный адаптер для Typst-отчётов.

## Как работает workflow

1. Агент читает методичку, исходные данные и текущий контекст.
2. Выделяет требования и проверяемые результаты.
3. Создаёт код, вычисления и артефакты в понятной структуре проекта.
4. Собирает отчёт и проверяет его по исходным требованиям.

`labflow` не придумывает метаданные титульного листа, требования преподавателя
или правила сдачи. Такие сведения должны приходить из задания, методички или
подключённого вузовского skill.

## Typst

`labflow-typst` создаёт структуру отчёта из контекста задания и не требует
передавать метаданные отдельными флагами. Скрипт и шаблоны находятся внутри
самого skill:

```text
skills/labflow-typst/
├── SKILL.md
├── scripts/init_typst.py
└── templates/gost/
```

## Соразмерные правки и review

Полная работа проходит все фазы и независимую проверку. Для локальной правки
абзаца, заголовка или слайда проверяются изменённый документ и зависимая
пагинация, без перезапуска несвязанного кода. Это не означает одобрение всего проекта.
Перед публикацией изменённого кандидата обязательные review выполняются заново;
прерванный, отсутствующий или устаревший результат не считается успешным.

ГУАП-специфичный адаптер `labflow-guap`, включая защищённый титульник и генератор,
находится отдельно в [guap-skill](https://github.com/pank-su/guap-skill/tree/main/skills/labflow-guap).
Универсальный Typst-инициализатор не должен перезаписывать такой шаблон.

## Проверяемое состояние работы

В `labflow` есть локальный [runtime](skills/labflow/references/runtime.md):

- карта требований с реальными фрагментами источников, страницами/строками и SHA-256;
- паспорт входов/выходов и независимых review: изменение файлов отзывает допуск;
- возобновляемые пакеты с per-item состояниями, дешёвым preflight и отзывом старых callbacks;
- отдельные состояния локальной проверки, независимого допуска и доставки.

Это библиотека/CLI для доверенного агента, не сетевой сервис и не sandbox.
Расписание, авторизация и отправка остаются у вызывающего адаптера.
Профили физлабы, расчётной, курсовой и конспекта находятся в `labflow-guap`.

## Разработка

Для всех проверок нужны Python 3.10+, PyYAML, PyMuPDF и Typst:

```bash
uv run --python 3.11 --with pyyaml --with pymupdf python -m unittest discover -s tests -v
```

Fixtures синтетические, без приватных переписок. Сквозные тесты реально выполняют
вычисления и сборку, но не выдают синтетического reviewer за независимую LLM-оценку.

## Структура репозитория

```text
skills/
├── labflow/
├── labflow-context/
├── labflow-coding/
├── labflow-math/
├── labflow-report/
├── labflow-self-review/
└── labflow-typst/
```

## Лицензия

MIT
